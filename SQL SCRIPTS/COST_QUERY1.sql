...
GROUP BY ROLLUP(EXTRACT(MONTH FROM L.L_SHIPDATE), EXTRACT(QUARTER FROM L.L_SHIPDATE), 
	EXTRACT(YEAR FROM L.L_SHIPDATE), P.P_TYPE, N1.N_NAME, N2.N_NAME)
ORDER BY Year, Quarter, Month, Type, "Exporting nation", "Importing nation";
--cost before indexes : cost=1582234.21..1582366.48 rows=52906 width=165) actual time=20321.076..20321.202 rows=2249 loops=1
--cost after indexes :  cost= 937902.40.. 938034.66 rows=52906 width=165) actual time=19017.393..19017.528 rows=2249 loops=1